...
variable trestart       equal   $(floor(v_ttotal/100))    	#Restart configuration saving period\n''')
    ...
    fo.write('variable tcycleMB   equal   $(floor((v_tbonds)/'+str(2*NumFixCycl)+'))    	#run time for each bond-equilibrating cycle\n')
    fo.write('variable tcycleRel   equal   $(floor((v_trelax)/'+str(2*NumFixCycl)+'))    	#run time for each relaxation stage cycle\n')
    fo.write(
        '''
